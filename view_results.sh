...
do 
cd $i
vmd -e ../../autodock_pipeline/viewing_results_state.vmd $( ls -v *pdb |  grep poses_[0-9][0-9][0-9])
...
export VMDNOCUDA=1
export VMDNOOPTIX=1
vmd -dispdev text  $(ls -v *poses_[0-9][0-9][0-9]*pdb | head -n 1 ) -e ../../autodock_pipeline/calculate_close_residues.tcl  &> /dev/null
...
readlink -f $(ls -v *poses_[0-9][0-9][0-9]* | head -n 1 ) >> /tmp/cat_results2.txt
