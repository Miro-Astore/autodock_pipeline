...
#all of the ew you can do better figure out how to call this stuff from vmd, like this bash script. john stone says it's possible and he knows everything. the purpose of this is to rename the drug residue to something reasonable so we can play with it in vmd.
...
#needs to be j or k for pose numbering
			...
			cat $f1\.pdb | grep -v '^END$' | grep -v '^ENDMDL' >> $out\_poses_$k.pdb
